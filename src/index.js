...
function setMode(mode,persist=true){workspace.dataset.mode=mode;for(const b of buttons)b.setAttribute("aria-pressed",String(b.dataset.modeButton===mode));if(mode!=="edit"){renderMarkdown(textarea.value);syncPreviewScroll()}if(persist)persistViewMode(mode);if(mode!=="preview")textarea.focus()}
